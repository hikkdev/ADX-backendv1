...
| `agreements` | AgreementTemplate (draft / live / superseded, `requiresReacceptance` — Lot D Q55) and AgreementAcceptance for the transaction kinds (Lot D Q123): INSERTION_ORDER per campaign (rendered server-side, through `/advertisers/:id/agreements/insertion-order`), PACKAGE_SALE per sale, JOB_TERMS per order on the agent's own tap, AGENT_PUBLISHER_PLATFORM / AGENT_ADVERTISER_PLATFORM once per applicant before review (AG-1); `isCurrentAcceptance` is the one re-acceptance rule the platform gates apply; `GET /agreements/stale` is the stale-terms report; DS-1→DS-4 (Digio eSign, 22 Sep 2026): SigningRequest — one e-signature request per party per document (`esign/`: the PDF rendered from the live template with merge fields, Digio asked for the signer sequence with the optional ADX countersign and e-stamp, the webhook / refresh moving it, the signed copy and audit certificate stored as SIGNED_AGREEMENT, the acceptance row written DIGIO on completion, completion hooks per kind, the hourly expiry sweep); the policy is platform settings `esign` (off by default) read through a port, the messages through `notify` through a port; the three signed-only kinds EMPLOYEE_APPOINTMENT, PRINT_PARTNER_SERVICE, PUBLISHER_LICENCE | `/agreements/*`, `/agreements/signing/*`, `/webhooks/digio/esign` | Platform — the words every party accepts, and who accepted which version |
| `agent-locations` | LT-1 (22 Sep 2026): AgentTrail + AgentLocationPoint — the agent's position on a job (the live fix in Redis `agentloc:last:<id>`, the trail in Postgres, thinned 25 m / 60 s; arrival stamped inside the geofence; the order's own position kept through `OrderPositionPort`), the ops live map (states, alerts idle / late / off-route / offline, ETA, trails, an SSE `snapshot` stream on a single-use token), the per-order timeline across the print and install legs, the nightly retention sweep (`tracking.retentionDays`); platform settings `tracking`; feature `ops.live-map` | `/agent-locations/*` | Ops — where every working agent is |
...
| `agents` | AgentProfile, AgentTierEvent, AgentRating, AgentMilestone, MilestoneTemplate; AG-1 (20 Sep 2026): the application ladder on `AgentProfile.stage` (APPLIED → … → UNDER_REVIEW → ACTIVE / ON_HOLD / REJECTED / WITHDRAWN / EXITED), the desk-set grade (G1–G4) beside the tier, the engagement (GIG / CONTRACT, dates, probation, reporting manager), the applicant's own details, `AgentDocument` (one row per paper with its own decision, numbers masked + hashed), AgentEducation / AgentEmployment / AgentReference / AgentPlatformExperience; `agentAcceptsWork` reads the stage; AG-4: `AgentInterview`, the desk's screen tick, `heldFromStage`, the paper-expiry sweep (`agent-document-expiry`), Cashfree's vehicle-RC check on a paper; AG-5: `FleetPartner` / `FleetInvite` (provenance on `fleetPartnerId`), the routing settings (AppConfig `agent-routing`), the exit's settlement through the port, the purge; `docs/agent-onboarding.md` | `/agents/*` (incl. `/me/tier`, `/me/leaderboard`, `/tier-ladder`, `/leaderboard`), `/milestones/*` | Agent experience |
...
| `training` | TrainingResource (the library), TrainingModule (AG-4: `audience` per side, `kind` LESSON / ASSESSMENT, `timeLimitMins`), TrainingQuestion/Option, AgentTrainingProgress, TrainingAttempt, AgentCertification (the lessons alone) | `/training/*` | Agent experience — DR 05; moved out of `agents` |
...
| `branding` | BrandRelease (QR-11) — the published brand, one row per release; the draft is the integrations row's `branding` section | `/branding`, `/branding/{draft,publish,releases*}` | Settings › Brand & theme: draft → publish → history → restore; the live brand every surface reads is `shared/integrations`' `getBrand`, served by app-config's public `GET /app/branding` |
...
| Outreach adapters | `shared/outreach/` | LH6 (D5): one adapter per provider — `whatsapp.ts` (Gupshup / Interakt / Meta Cloud API), `meta-dm.ts` (Instagram DM + Messenger over the Graph API, comment-to-DM and story replies read off the webhook), `google-business.ts` (service-account JWT, HMAC-SHA512 webhook), `telephony.ts` (Exotel / Knowlarity / Twilio Voice: click-to-call agent-first on a masked number, the status and inbound hooks, `answerTwiml` / `ivrTwiml`, `fetchRecording`). Each answers `describe()` (configured + missing fields), a `SendOutcome` (never throws for a provider's no), and `OutreachWebhookRejected` on an untrusted hook. Reachability, the window rule, the two comms rules and attribution are the hub's (`modules/leads/outreach.service.ts`). |
...
| QR engine seam | `shared/qr-engine/` | QR-1 (16 Sep 2026): the `LOCAL \| GENQR` port behind every code that goes to print and every hosted dynamic code. `local.ts` is the `qrcode` package in the house style (colours and size, `styled: false`); `genqr.ts` is our own QR platform — GenQR, its own repository, deployment and database, sold to the public — reached as a tenant over `/api/v1` with a scoped Enterprise key: `POST /render` (a styled SVG of content ADX owns, nothing stored), `POST /qrcodes/batch` (one dynamic code per campaign hoarding, 500 a call, atomic), `GET /qrcodes/:id/analytics`, `GET /qrcodes/:id/image.(svg\|png)`, `GET /me` (the probe). The door: `renderPrinted` (GenQR, else LOCAL — a print job never fails to draw; the fallback is logged), `renderDynamic` (GenQR's image, else the STORED short URL drawn locally, else `/t/`), `registerDynamicCodes` (503 when nothing hosts), `retireDynamicCode`, `dynamicCodeAnalytics` (a code the engine cannot answer for is absent, never a throw), `testQrEngine`. Config `qrEngine` on the integrations row (`provider`, `baseUrl`, `apiKey`, `shortBaseUrl` — the ADX-branded origin a hoarding carries, set as "Redirect base" on the GenQR account and proxied to GenQR's `/r/` — and the print `style`), env `GENQR_BASE_URL` / `GENQR_API_KEY` / `GENQR_SHORT_BASE_URL`. The scan path the owner chose: `GenQR /r/XXXX → ADX /t/XXXX → destination` — ADX stays the counter it always was; GenQR adds the artwork, the short printed URL and its own breakdowns, drawn beside ADX's number as `engine` (provenance ENGINE), never in its place. The ninety-second onboarding and grant codes never touch it. Errors by GenQR's table: 401 / 403 `insufficient_scope` → 503 `INTEGRATION_NOT_CONFIGURED` with GenQR's sentence; 403 `quota_exceeded` → 409; 429; else 502. The key goes in the header and nowhere else. README in the folder. |
