...
describe('ensureAgreementDrafts', () => {
  it('seeds version 1 of each of the five kinds as a marked DRAFT on an empty database', async () => {
    const created = await ensureAgreementDrafts();
    expect(created).toEqual(['PLATFORM', 'ADVERTISER_PLATFORM', 'INSERTION_ORDER', 'PACKAGE_SALE', 'JOB_TERMS']);
  ...
  it('is idempotent and never touches a kind that already has a row — draft, live or superseded', async () => {
    ...
    expect(first).toEqual(['ADVERTISER_PLATFORM', 'PACKAGE_SALE', 'JOB_TERMS']);
    ...
    expect(repository.createTemplate).toHaveBeenCalledTimes(3);
  });
