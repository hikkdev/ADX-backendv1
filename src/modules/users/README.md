...
| DELETE | `/me/sessions` | `authenticate` — E6: ends every other session and keeps this one; `{ revoked: number }`, the count of sessions ended (E11-1: pinned by test, zero when this was the only device). Only the refresh tokens go (the per-user revocation marker would take the caller's own access token), so the other devices' access tokens lapse within one lifetime, as a single revoke already behaves; 409 for a token that names no session |
| DELETE | `/me/sessions/:id` | `authenticate` |
| GET | `/me/activity` | `authenticate` |
| GET | `/me/contacts` | `authenticate` — K-B1: `{ primary: { mobile, mobileVerifiedAt, email, emailVerified }, contacts: [{ id, kind, value, label, verifiedAt, addedBy: { id, name }, createdAt }] }`. `emailVerified` is derived from the codes the address has answered (`auth.hasProvenEmail`) — `User` carries no `emailVerifiedAt` |
| POST | `/me/contacts` | `authenticate` (**201**) — `{ kind: EMAIL\|PHONE, value, label? }`; normalised (lower-cased email, E.164 number); starts unverified; **409 `CONTACT_TAKEN`** (`details.which`: PRIMARY \| CONTACT, `userId`, `contactId?`) when the value is any account's primary or any contact row |
...
| POST | `/:id/contacts/:contactId/verify` | ADMIN — `{ code }` the person read back; audited `USER_CONTACT_VERIFIED` (`how: CODE`) |
| POST | `/:id/contacts/:contactId/mark-verified` | ADMIN — `{ reason }`: the desk's word, no code typed; audited `USER_CONTACT_MARKED_VERIFIED` (`how: READ_BACK`) |
| POST | `/:id/contacts/:contactId/make-primary` | ADMIN — `{ reason }`; the same swap as the person's, and the desk **may promote an unverified contact** — the audit row (`USER_PRIMARY_CHANGED`, with the before/after pair, the reason and the admin) says `verified: UNVERIFIED` and the new primary carries no `mobileVerifiedAt`; 409 `CONTACT_TAKEN` when the value has since become another account's. Lot K2: **403 `USE_YOUR_OWN_SETTINGS` when `:id` is the acting admin's own account** — an admin proves their own contact like everyone else, from `/me/contacts` |
...
Closure and erasure (Lot A, Q21/Q60) hang off `/users` but are owned by
...
registration, owned by `notifications`, see `modules/notifications/README.md`)
and `/me/data-export` (POST / GET — the person's data export, owned by
`account-lifecycle`, see its README).
...
`User.mobile` (the sign-in identity, unique) and `User.email` (unique) stay
the primary pair. A `UserContact` row (`kind` EMAIL | PHONE, `value`
normalised, `label`, `verifiedAt`, `addedById`) is every other number or
...
- **One value, one account.** A mobile or email is free only when no account
  ...
  **case-insensitive** (`lower() = lower()` in the repository, not Prisma's
  `mode: 'insensitive'`, whose ILIKE would read `_` and `%` in the value as
  wildcards) because the unique index is not and rows written before K-B1
  ...
  rows differing only by case are reported and left for a person). And a
  same-instant add that slips past the check is caught by the
  `@@unique(kind, value)` index and answered as the same 409 `CONTACT_TAKEN`.
- **A contact starts unverified.** It is proved with a code (auth's senders,
  ...
  value now that the migration has landed), or marked verified by an admin
...
- **Make-primary is one transaction** (`repository.swapPrimary`): the
  ...
  it), the primary column moves. A PHONE promotion then runs
...
address has answered (`auth.hasProvenEmail`): `User` carries no
`emailVerifiedAt` column, which is the honest gap.

## Owned Prisma entities

`User` (profile columns), `UserRole`, `ImpersonationSession`, K-B1:
...
module through `orders`) — the same deliberate cross-domain read the deletion
...
- `findUserLabels(ids)` — E6: `Map<id, { id, name }>` in one query, for the
  ...
  module). A name is the user's name, else their mobile, else null.
- `findUserSummaries(ids)`, `primaryRoleOf(roles)` — E7-3: `Map<id, { id,
  ...
  requester rail; the impersonation list here). `role` is the primary role
  the console prints — party roles first, ADMIN last, null for none. An id
  nobody matches is absent from the map.
- `ensureSystemUser()`, `systemUserId()`, `SYSTEM_USER_MOBILE`,
  `SYSTEM_USER_NAME` — E6: the system account (`+910000000000`, `ADX
  system`, `isActive: false`, no roles) the jobs write their audit rows under
...
- `app-config` — `getFlow`, `onboardingTemplateSchema` (Q83: the ladder as data).
...
- **`GET /users/me` carries the second-factor state** — `twoFactorRequiredAt`,
  `emailOtpFallbackCount`, `emailOtpFallbackResetAt` (E6) — on `/me` only, so
...
- **An admin's mobile number is never changed from the desk** — 409
  ...
  in as them.
- **Anybody else's number needs a `reason`** (400 without one), ends every
  ...
  code's `userId`), and is logged as `MOBILE_CHANGED_BY_ADMIN` with a diff. The reason
...
- The **last active super admin** cannot be deleted, deactivated or closed
  ...
  on every door, counted on the members who can still sign in).
