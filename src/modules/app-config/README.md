...
| GET | `/api/v1/app/branding` | **none** — QR-9 (17 Sep 2026): the brand every surface draws, `{ platformName, tagline, primaryColor, deepColor, inkColor, groundColor, wordmarkUrl, wordmarkInverseUrl, markUrl, markInverseUrl, iconUrl, defaults[], version }` from `shared/integrations`' `getBrand(baseUrl)` — DR 11 (`#E40209` / `#BD2020` / `#0F0F0F` / `#F5F5F5`, "Space that gets seen.", the SVGs under `/brand/*` that `create-app.ts` serves static from `public/brand`, 7-day immutable) with whatever Settings › Branding stored laid over it; `defaults` names the fields still on DR 11; `version` is a hash of the resolved brand so a client swaps logos only when something changed. `Cache-Control: public, max-age=300`. Read before sign-in by both apps (`readBrand` at launch) and the console (`BrandProvider`) |
