...
| GET | `/api/v1/integrations` → `qrEngine` | QR-1: `{ provider: LOCAL \| GENQR, baseUrl, apiKey (masked), shortBaseUrl, style: { foregroundColor?, backgroundColor?, dotStyle?, frameStyle?, frameCaption?, logoUrl? }, hostsDynamic }` — `hostsDynamic` is the verdict the campaign screens read: GENQR chosen AND a host and key present (the seam's `dynamicCodesAvailable` from the same fields) |
