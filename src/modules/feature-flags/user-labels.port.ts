/**
 * E6: who moved a flag, by name.
 *
 * `users` sits above this module in the graph (it reaches `advertisers`,
 * which reaches `payouts`, which reads the flags), so the name lookup is
...
export function registerFlagUserLabelPort(port: UserLabelPort): void {
  registered = port;
}
