...
const publishers: Format = {
  ...
  columns: [
    ...
    // QR-13: the person behind the account and the address pin — a row naming a person opens (or adopts) the account with the publisher.
    col('firstName', false, 'text', 'The owner, who signs in: their first name. Given, the account is opened with the row and the app never asks their name.', 'Asha', { maxLength: 60 }),
    col('lastName', false, 'text', 'Their last name.', 'Rao', { maxLength: 60 }),
    col('dateOfBirth', false, 'date', 'Their date of birth, YYYY-MM-DD, 18 or over — one of the four readiness basics.', '1980-05-14'),
    col('gender', false, 'enum', 'Optional.', 'FEMALE', { enumValues: ['MALE', 'FEMALE', 'OTHER', 'PREFER_NOT_TO_SAY'] }),
    col('latitude', false, 'number', 'The address pin, decimal degrees; with the longitude.', '18.5204'),
    col('longitude', false, 'number', 'With the latitude.', '73.8567'),
  ],
  rules: [MOBILE_RULE, "A row with a first name opens the owner's account (or adopts the one on the number) and, with name, email, address and date of birth in, lands ONBOARDING_COMPLETE; a merge never touches an existing account or pin.", 'A PAN or GSTIN already on another publisher is a WARNING and the row still creates.', 'An unknown city is a WARNING, kept as typed.', CASING_RULE, INVALID_RULE, NEVER_REFUSED],
  sampleRows: [
    { name: 'Metro Spaces Pvt Ltd', mobile: '9876543210', email: 'ops@metrospaces.in', type: 'BUSINESS', gstin: '27ABCDE1234F1Z5', address: '12, MG Road, Pune', city: 'Pune', state: 'Maharashtra', contactName: 'Asha Rao', contactMobile: '9876500000', contactEmail: 'asha@metrospaces.in', panNumber: 'ABCDE1234F', firstName: 'Asha', lastName: 'Rao', dateOfBirth: '1980-05-14', gender: 'FEMALE', latitude: '18.5204', longitude: '73.8567' },
    { name: 'Ravi Kulkarni', mobile: '9822012345', email: '', type: 'INDIVIDUAL', gstin: '', address: '', city: 'Nashik', state: 'Maharashtra', contactName: '', contactMobile: '', contactEmail: '', panNumber: '', firstName: '', lastName: '', dateOfBirth: '', gender: '', latitude: '', longitude: '' },
  ],
...
const advertisers: Format = {
  ...
  columns: [
    ...
    col('firstName', false, 'text', "The person's first name. Given, the row opens their sign-in account up front (with lastName, address and city — a companyName too unless INDIVIDUAL), as the desk does.", 'Meera', { maxLength: 60 }),
    col('lastName', false, 'text', "The person's last name.", 'Shah', { maxLength: 60 }),
    col('dateOfBirth', false, 'text', 'YYYY-MM-DD, 18 or over; on the account, not the profile.', '1988-02-14'),
    col('gender', false, 'enum', 'On the account, not the profile.', 'FEMALE', { enumValues: ['MALE', 'FEMALE', 'OTHER', 'PREFER_NOT_TO_SAY'] }),
  ],
  rules: [MOBILE_RULE, 'A PAN or GSTIN already on another advertiser is a WARNING and the row still creates.', 'panNumber and contactName are validated and reported but not written.', "A row with a firstName is a desk onboarding: the sign-in account is opened with the number and the ADVERTISER role, so the person's first sign-in has nothing to ask.", CASING_RULE, INVALID_RULE, NEVER_REFUSED],
  sampleRows: [
    { name: 'Fresh Foods', mobile: '9000000001', email: 'hello@freshfoods.in', type: 'COMMERCIAL', companyName: 'Fresh Foods Pvt Ltd', industry: 'Retail', gstin: '27ABCDE1234F1Z5', panNumber: 'ABCDE1234F', address: '4, FC Road, Pune', city: 'Pune', state: 'Maharashtra', contactName: 'Meera Shah', firstName: 'Meera', lastName: 'Shah', dateOfBirth: '1988-02-14', gender: 'FEMALE' },
    { name: 'Sunita Patil', mobile: '9000000002', email: '', type: 'INDIVIDUAL', companyName: '', industry: '', gstin: '', panNumber: '', address: '', city: 'Mumbai', state: '', contactName: '', firstName: '', lastName: '', dateOfBirth: '', gender: '' },
  ],
